...
export async function GET(
  ...
  if (currentDay >= card.billGenerationDate) {
    ...
    billEndDate = new Date(currentYear, currentMonth + 1, card.billGenerationDate - 1);
    ...
    billEndDate = new Date(currentYear, currentMonth, card.billGenerationDate - 1);
  ...
  const existingInvoice = await db.invoice.findFirst({
    where: {
      creditCardId: id,
      billStartDate: { gte: new Date(billStartDate.getFullYear(), billStartDate.getMonth(), billStartDate.getDate()) },
      billEndDate: { lte: new Date(billEndDate.getFullYear(), billEndDate.getMonth(), billEndDate.getDate(), 23, 59, 59) },
    },
  });
