...
test.describe("UI Consistency Tests", () => {
  ...
  test("Home page has consistent layout and footer padding", async ({
    page,
  }) => {
    await page.goto("/");
    await page.waitForLoadState("networkidle");
    ...
    if ((await cardLink.count()) > 0) {
      ...
      if ((await transactions.count()) > 0) {
        ...
        if (transactionBox && footerBox) {
          expect(transactionBox.y + transactionBox.height).toBeLessThan(
            footerBox.y,
          );
        }
      }
  ...
  test("Footer does not hide action buttons", async ({ page }) => {
    ...
    const floatingButtons = page.locator(
      'button[class*="fixed"], button[aria-label*="Add"], button[aria-label*="transaction"]',
    );
