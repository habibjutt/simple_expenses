...
export default function TransactionModal({
  ...
  const [paymentType, setPaymentType] = useState<"creditCard" | "bankAccount">(
    "bankAccount",
  );
  ...
  useEffect(() => {
    if (editTransaction && open) {
      ...
      setRecurringFrequency(
        (editTransaction.recurringFrequency as RecurringFrequency) ?? "monthly",
      );
      setRecurringEndDate(
        editTransaction.recurringEndDate
          ? new Date(editTransaction.recurringEndDate)
              .toISOString()
              .split("T")[0]
          : "",
      );
      if (editTransaction.isRecurring || editTransaction.notes)
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (transactionType === "transfer") {
        ...
        if (paymentType === "creditCard" && !creditCardId)
          throw new Error("Please select a credit card");
        if (paymentType === "bankAccount" && !bankAccountId)
            ...
            {transactionType === "transfer" ? (
              ...
              <>
                ...
                <div className="grid grid-cols-2 gap-3">
                  <Field
                    label={
                      transactionType === "income" ? "Receive To" : "Pay With"
                    }
                    required
                  >
                    <FormCombobox
                        ...
                        <div className="flex items-center justify-between">
                          ...
                          <button
                            ...
                            onClick={() => {
                              ...
                              if (isInstallments) setInstallments("1");
                            }}
                            className={cn(
                              ...
                              {recurringFrequency === "daily"
                                ...
                                : recurringFrequency === "weekly"
                                  ...
                                  : recurringFrequency === "monthly"
                                    ? "month"
                                    : "year"}
                              . You can pause or stop this anytime.
                            </p>
                          </div>
                        )}
                      </div>
            ...
            <DialogFooter>
              ...
              >
                {loading
                  ...
                  : editTransaction
                    ? "Update Transaction"
                    : `Add ${typeConfig[transactionType].label}`}
              </Button>
            </DialogFooter>
          </form>
        </DialogContent>
