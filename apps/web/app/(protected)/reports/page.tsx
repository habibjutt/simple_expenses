...
export default function ReportsPage() {
  ...
  const [tab, setTab] = useState<"categories" | "trend" | "budget">(
    "categories",
  );
  ...
  const handleExportReportCsv = async () => {
    try {
      ...
      const fallback =
        tab === "trend"
          ? `report-trend-${year}.csv`
          : tab === "budget"
            ? `report-budget-${year}-${mPad}.csv`
            : `report-categories-${year}-${mPad}.csv`;
            ...
            <div className="flex gap-2 mt-4">
              {[
                ...
                <button
                  key={t.id}
                  onClick={() =>
                    setTab(t.id as "categories" | "trend" | "budget")
                  }
        ...
        <div className="max-w-7xl mx-auto px-4 md:px-6 mt-5">
          <div className="flex justify-end mb-3">
            <Button
              ...
              className="flex items-center gap-2 h-9 border-slate-200 bg-white hover:bg-slate-50 shadow-sm"
          ...
          {loading ? (
            ...
            <>
              ...
              <div className="grid grid-cols-3 gap-3 mb-5">
                ...
                <div
                  className={`rounded-2xl p-4 shadow-sm border text-center ${net >= 0 ? "bg-emerald-50 border-emerald-200" : "bg-red-50 border-red-200"}`}
                >
              ...
              {expenseStats.length > 0 ? (
                ...
                <EmptyState
                  icon={PieIcon}
                  title={`No expense data for ${monthLabel}`}
          ...
          budgetData.length === 0 ? (
            <EmptyState
              ...
              description="Set spending limits to track your budget vs actual spending"
              actionLabel="Go to Spending Limits"
              actionHref="/spending-limits"
              actionStyle="link"
            />
          ) : (
            <div className="bg-white rounded-2xl shadow-sm border border-slate-200/60 overflow-hidden">
              ...
              <div className="px-5 py-4 bg-slate-50 border-t border-slate-100">
                ...
                <div className="flex justify-between text-sm mt-1">
                  ...
                  <span
                    className={`font-semibold tabular-nums ${budgetData.reduce((s, i) => s + i.actual, 0) > budgetData.reduce((s, i) => s + i.budget, 0) ? "text-red-600" : "text-[#1a9e5c]"}`}
                  >
                    {formatCurrency(
